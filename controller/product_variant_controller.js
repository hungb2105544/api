...
class ProductVariantController {
  ...
  static async handleBulkCreateWithSharedImages(req, res) {
    try {
      // Parse variantsData from FormData
      const variantsData = req.body.variantsData
        ? JSON.parse(req.body.variantsData)
        : [];
      const imageFiles = req.files || [];
      const userId = req.user?.id; // Giả sử middleware xác thực cung cấp userId
      ...
      if (!variantsData || variantsData.length === 0) {
        return res.status(400).json({
          success: false,
          message:
            "Dữ liệu biến thể (variantsData) là bắt buộc và không được rỗng.",
        });
      }

      const createdVariants =
        await ProductVariantModel.createVariantsWithSharedImages(
          variantsData,
          imageFiles,
          userId
        );
      ...
      return res.status(201).json({
        success: true,
        message: "Tạo hàng loạt biến thể thành công",
        data: createdVariants,
      });
    } catch (error) {
      const statusCode = error.message.includes("tồn tại") ? 400 : 500;
      console.error(
        "❌ Controller - Lỗi khi tạo biến thể hàng loạt:",
        error.message
      );
      return res.status(statusCode).json({
        success: false,
        message: error.message || "Lỗi khi tạo biến thể hàng loạt",
      });
    }
  }
}
